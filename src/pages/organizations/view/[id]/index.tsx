...
import { deleteFlashcardDeckById } from 'apiSdk/flashcard-decks';
  ...
  const { data, error, isLoading, mutate } = useSWR<OrganizationInterface>(
    ...
    () =>
      getOrganizationById(id, {
        relations: ['user', 'flashcard_deck'],
      }),
  );

  const flashcard_deckHandleDelete = async (id: string) => {
    setDeleteError(null);
    try {
              ...
              {hasAccess('flashcard_deck', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                <Stack spacing={2} mb={8}>
                  <Flex justifyContent="space-between">
                    ...
                    <NextLink passHref href={`/flashcard-decks/create?organization_id=${data?.id}`}>
                  ...
                  <TableContainer mb={4}>
                    <Table variant="simple">
                      ...
                      <Tbody>
                        {data?.flashcard_deck?.map((record) => (
                          <Tr
                            cursor="pointer"
                            onClick={() => router.push(`/flashcard-decks/view/${record.id}`)}
                            key={record.id}
                          >
                            <Td>{record.title}</Td>
                            <Td>
                              {hasAccess('flashcard_deck', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                                <NextLink
                                  onClick={(e) => e.stopPropagation()}
                                  passHref
                                  href={`/flashcard-decks/edit/${record.id}`}
                                >
                              ...
                              {hasAccess('flashcard_deck', AccessOperationEnum.DELETE, AccessServiceEnum.PROJECT) && (
                                <IconButton
                                  onClick={(e) => {
                                    e.stopPropagation();
                                    flashcard_deckHandleDelete(record.id);
                                  }}
                                  colorScheme="red"
                                  variant="outline"
                                  aria-label="edit"
