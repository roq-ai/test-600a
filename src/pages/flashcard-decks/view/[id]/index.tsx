...
import { getFlashcardDeckById } from 'apiSdk/flashcard-decks';
import { Error } from 'components/error';
import { FlashcardDeckInterface } from 'interfaces/flashcard-deck';
import useSWR from 'swr';
import { useRouter } from 'next/router';
import { AccessOperationEnum, AccessServiceEnum, useAuthorizationApi, withAuthorization } from '@roq/nextjs';
import { deleteFlashcardById } from 'apiSdk/flashcards';

function FlashcardDeckViewPage() {
  ...
  const { data, error, isLoading, mutate } = useSWR<FlashcardDeckInterface>(
    ...
    () =>
      getFlashcardDeckById(id, {
        relations: ['organization', 'user', 'flashcard'],
      }),
  );

  const flashcardHandleDelete = async (id: string) => {
    setDeleteError(null);
    try {
      ...
      <Box bg="white" p={4} rounded="md" shadow="md">
        <Flex justifyContent="space-between" mb={4}>
          <Text as="h1" fontSize="2xl" fontWeight="bold">
            Flashcard Deck Detail View
          </Text>
          {hasAccess('flashcard_deck', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
            <NextLink href={`/flashcard-decks/edit/${data?.id}`} passHref legacyBehavior>
              <Button
              ...
              {hasAccess('flashcard', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                <Stack spacing={2} mb={8}>
                  <Flex justifyContent="space-between">
                    ...
                    <NextLink passHref href={`/flashcards/create?flashcard_deck_id=${data?.id}`}>
                  ...
                  <TableContainer mb={4}>
                    <Table variant="simple">
                      ...
                      <Tbody>
                        {data?.flashcard?.map((record) => (
                          <Tr
                            cursor="pointer"
                            onClick={() => router.push(`/flashcards/view/${record.id}`)}
                            key={record.id}
                          >
                            ...
                            <Td>
                              {hasAccess('flashcard', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                                <NextLink
                                  onClick={(e) => e.stopPropagation()}
                                  passHref
                                  href={`/flashcards/edit/${record.id}`}
                                >
                              ...
                              {hasAccess('flashcard', AccessOperationEnum.DELETE, AccessServiceEnum.PROJECT) && (
                                <IconButton
                                  onClick={(e) => {
                                    e.stopPropagation();
                                    flashcardHandleDelete(record.id);
                                  }}
                                  colorScheme="red"
                                  variant="outline"
                                  aria-label="edit"
